...
namespace UpgradeSystem
{
    ...
    public class UpgradeData : IBinarySerializable
    {
        ...
        {
            if (CanAfford(amount)) upgradeCurrency = Math.Max(0, upgradeCurrency - amount);
        }
    }
    ...
    public class CommonWeaponUpgrade
    {
        ...
        {
            if (ByteSize != data.Length) throw new Exception($"Expected {ByteSize}bytes got {data.Length}bytes");
